...
function translate_smiley( $matches ) {
	...
	$images = $wpsmiliestrans[ $smiley ];

	$matches = array();
	$ext = preg_match( '/\.([^.]+)$/', $images, $matches ) ? strtolower( $matches[1] ) : false;
	 ...
	 * @param string $images        Filename for the smiley image.
	 * @param string $site_url   Site URL, as returned by site_url().
	 */
	$src_url = apply_filters( 'smilies_src', includes_url( "images/smilies/$images" ), $images, site_url() );

	return sprintf( '<images src="%s" alt="%s" class="wp-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
...
/**
 * Convert emoji to a static images element.
 *
 * @since 4.2.0
 *
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' ==  $ignore_block_element && strlen( $content ) > 0 && '<' != $content[0] && false !== strpos( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<images src="%s" alt="%s" class="wp-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
