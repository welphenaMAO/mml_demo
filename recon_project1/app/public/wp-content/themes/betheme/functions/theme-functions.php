...
//				$output .= '</div>'."\n";
                ...
                $output .= '<div class="pn-btn">';

                if( $paged >1 ){
                    $output .= '<a class="prev-page" href="'. get_pagenum_link( $prev ) .'"><i class="fas fa-angle-left"></i></a>';
                }

                $output .= '<ul>';
                for( $i=1; $i <= $total; $i++ ){
                    if ( $i == $current ){
                        $output .= '<li class="active"><a href="'. get_pagenum_link($i) . '">' . $i .'</a></li>';
                        $dots = true;
                    } else {
                        if ( $show_all || ( $i <= $end_size || ( $current && $i >= $current - $mid_size && $i <= $current + $mid_size ) || $i > $total - $end_size ) ){
                ...
                if( $paged < $total ){
                    $output .= '<a class="next-page" href="'. get_pagenum_link( $next ) .'"><i class="fas fa-angle-right"></i></a>';
                }
