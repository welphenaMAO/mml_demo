...
tinymce.PluginManager.add( 'wordpress', function( editor ) {
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<images src="' + tinymce.Env.transparentSrc + '" data-wp-more="more" data-wp-more-text="' + moretext + '" ' +
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<images src="' + tinymce.Env.transparentSrc + '" data-wp-more="nextpage" class="wp-more-tag mce-wp-nextpage" ' +
	...
	// Display the tag name instead of images in element path
	...
	editor.addCommand( 'WP_More', function( tag ) {
		...
		title = __( title );
		html = '<images src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
